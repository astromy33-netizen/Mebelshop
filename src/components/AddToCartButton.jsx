...
export const AddToCartButton = ({ product, variant = 'default' }) => {
  ...
  if (variant === 'ozon') {
    return (
      <button
        onClick={handleAdd}
        className="w-full px-4 py-2 bg-gradient-to-r from-amber-400 via-orange-500 to-rose-500 text-slate-950 rounded-lg font-semibold text-sm shadow-[0_14px_30px_-18px_rgba(249,115,22,0.7)] hover:shadow-[0_18px_45px_-18px_rgba(244,63,94,0.75)] hover:-translate-y-0.5 transition-all duration-200"
      >
        {t('product.addToCart')}
      </button>
    );
